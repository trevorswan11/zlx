...
pub fn parseStmt(p: *parser.Parser) !ast.Stmt {
    return if (lus.stmt_lu.get(p.currentTokenKind())) |stmt_fn| {
        try stmt_fn(p);
    } else {
        try parseExpressionStmt(p);
    };
}

pub fn parseExpressionStmt(p: *parser.Parser) !ast.Stmt {
    const expression = expr.parseExpr(p, .DEFAULT_BP);
...
pub fn parseVarDeclStmt(p: *parser.Parser) !ast.Stmt {
    ...
    if (p.currentTokenKind() == .COLON) {
        ...
        explicit_type = try types.parse_type(p, .DEFAULT_BP);
    }

    var assignment_value: ?ast.Expr = undefined;
    if (p.currentTokenKind() != .SEMI_COLON) {
        _ = try p.expect(.ASSIGNMENT);
        assignment_value = expr.parseExpr(p,.ASSIGNMENT);
    } else if (explicit_type) |_| {
...
pub fn parseFnParamsAndBody(p: *parser.Parser) !FunctionInfo {
    ...
    while (p.hasTokens() and p.currentTokenKind() != .CLOSE_PAREN) {
        ...
        function_params.append(ast.Parameter{
            .name = param_name,
            .type = param_type,
        });

        if (!p.currentToken().isOneOfMany(@constCast(&[_]token.TokenKind{.CLOSE_PAREN, .EOF}))) {
...
pub fn parseIfStmt(p: *parser.Parser) !ast.Stmt {
    ...
    if (p.currentTokenKind() == .ELSE) {
        ...
        alternate = if (p.currentTokenKind() == .IF) {
            parseIfStmt(p);
        } else parseBlockStmt(p);
    }
